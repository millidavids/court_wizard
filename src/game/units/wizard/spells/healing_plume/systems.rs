...
/// Guest wizard healing plume casting -- reads network signals.
#[allow(clippy::too_many_arguments)]
pub fn handle_healing_plume_casting_guest(
    time: Res<Time>,
    mut commands: Commands,
    mut meshes: ResMut<Assets<Mesh>>,
    mut materials: ResMut<Assets<StandardMaterial>>,
    ...
    if cast_result.completed {
        // Spawn healing zone at cursor position for guest
        if let Some(pos) = clamped_cursor {
...
/// Core healing plume casting logic -- called by both local and guest systems.
///
