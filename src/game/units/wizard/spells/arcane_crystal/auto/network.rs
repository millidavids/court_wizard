...
use super::super::infusions::InfusionFamily;
...
pub(crate) fn crystal_network_chain(
    ...
    let crystal_data: Vec<(Entity, Vec3, Option<CrystalInfusion>, bool)> = crystals
        .iter()
        .map(|(e, c)| (e, c.position, c.infusion, c.just_absorbed))
        .collect();
    ...
    for (source_entity, source_pos, source_spell, source_pulsed) in &crystal_data {
        ...
        // Only projectile echoes can be duplicated into a chained crystal. Ticked
        // and continuous infusions run off the target crystal's own systems, so
        // chaining them here would double-apply; they still get the visual pulse.
        let chain_emits = remembered.family() == InfusionFamily::Emitter;
                ...
                if chain_emits {
                    match remembered {
                        CrystalInfusion::MagicMissile => {
                            ...
                            let mini_radius =
                                magic_missile_constants::COLLISION_RADIUS * SIZE_SCALE;
                        ...
                        CrystalInfusion::Fireball => {
                            let count =
                                scaled_count(MINI_FB_COUNT / 2 + 1, target_crystal.count_mult);
                        ...
                        CrystalInfusion::Meteor => {
                            ...
                            let mini_radius =
                                meteor_fall_constants::METEOR_MESH_RADIUS * SIZE_SCALE;
                            for (_, tp) in &meteor_targets {
                                ...
                                let entity = meteor_fall_systems::spawn_meteor_projectile_entity(
                                    ...
                                    Vec3::new(
                                        0.0,
                                        meteor_fall_constants::METEOR_INITIAL_VELOCITY,
                                        0.0,
                                    ),
                        ...
                        | CrystalInfusion::FingerOfDeath => {
                            // Beams are too complex to duplicate in a chain — pulse only.
                        }
                        other => unreachable!(
