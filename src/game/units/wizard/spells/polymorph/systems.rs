...
pub fn handle_polymorph_casting(
    ...
    let cast_result = polymorph_casting_logic(
        &input,
        &time,
        &mut casting_state,
        &mut mana,
...
/// Guest wizard polymorph casting — reads network signals.
#[allow(clippy::too_many_arguments)]
pub fn handle_polymorph_casting_guest(
    time: Res<Time>,
    mut commands: Commands,
    mut materials: ResMut<Assets<StandardMaterial>>,
    mut wizard_query: Query<
...
/// Core polymorph casting logic — called by both local and guest systems.
#[allow(clippy::too_many_arguments)]
