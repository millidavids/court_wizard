...
/// `damage_percent` is the fraction of each victim's max health to deal.
...
pub fn apply_necrotic_explosion_damage(
    ...
    // Collect positions and damage fractions of explosions that haven't applied damage yet
    ...
    for (transform, mut burst) in explosions.iter_mut() {
        if !burst.damage_applied {
            ...
            pending.push((transform.translation, burst.damage_percent));
        }
    }

    for (explosion_pos, explosion_damage_percent) in &pending {
        for (
            entity,
            transform,
            mut health,
