...
pub fn handle_hypnotic_pattern_casting(
    ...
    if primed_spell.spell != Spell::HypnoticPattern {
        return;
    }

    let completed = hypnotic_pattern_casting_logic(
        &input,
        &time,
        wizard_entity,
        wizard_transform,
...
fn hypnotic_pattern_casting_logic(
    ...
    caster_query: &Query<&SpellCaster>,
    indicator_query: &mut Query<&mut HypnoticPatternIndicator>,
    targets_query: &Query<(Entity, &Transform), Without<Corpse>>,
...
pub fn update_hypnotic_pattern_indicator(
    time: Res<Time>,
    mut indicators: Query<(&mut HypnoticPatternIndicator, &mut Transform)>,
) {
...
pub(crate) fn apply_mesmerize(
    ...
    let duration = constants::MESMERIZE_DURATION * empowerment;
    for (entity, transform) in targets.iter() {
        let distance = transform.translation.distance(circle_pos);
        if distance <= radius {
            commands
                .entity(entity)
                .insert(MesmerizedModifier::new(duration));
        }
    }
}
...
fn spawn_circle_indicator(
    ...
    commands
        .spawn((
            ...
            MeshMaterial3d(assets.hypnotic_pattern_indicator.clone()),
            ...
            HypnoticPatternIndicator::new(position, empowerment),
            OnGameplayScreen,
        ))
        .id()
}
