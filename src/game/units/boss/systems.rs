...
use super::constants::*;
use super::resources::BossAssets;
...
pub fn spawn_boss(mut commands: Commands, boss_assets: Res<BossAssets>) {
    ...
    let spawn_y = hitbox.height / 2.0 + (BOSS_ELLIPSE_DEPTH / 2.0) + 1.0;
    ...
    let initial_velocity = to_center.normalize_or_zero() * BOSS_MOVEMENT_SPEED;

    commands
        .spawn((
            // Rendering
            Mesh3d(boss_assets.mesh.clone()),
            MeshMaterial3d(boss_assets.material_phase0.clone()),
...
/// Updates boss targeting velocity toward nearest enemy.
...
/// Boss melee combat system — runs on its own cooldown timer (not the global attack cycle).
...
pub fn boss_combat(
    ...
    mut commands: Commands,
    mut bosses: Query<
        (Entity, &Transform, &Hitbox, &Team, &mut BossAttackCooldown),
    ...
    for (boss_entity, boss_transform, boss_hitbox, boss_team, mut attack_cooldown) in &mut bosses {
        ...
        for (entity, target_transform, target_hitbox, team, mut health, mut temp_hp) in &mut targets
        {
            ...
            // Apply damage
            apply_damage_to_unit(&mut health, temp_hp.as_deref_mut(), BOSS_ATTACK_DAMAGE);
            ...
            commands.entity(entity).insert(KnockbackEffect::new(
                direction,
                BOSS_MELEE_KNOCKBACK_SPEED,
                BOSS_MELEE_KNOCKBACK_DURATION,
            ));
        }
    }
}

/// Boss movement system using weighted velocities.
...
/// Updates the boss's enrage state based on HP thresholds.
...
pub fn update_enrage_state(
    ...
    for (health, mut enrage, mut damage_mult, mut mesh_material, original_material) in &mut bosses {
        ...
        if new_phase != enrage.phase {
            ...
            damage_mult.0 = BOSS_DAMAGE_MULTIPLIER + enrage.damage_bonus;

            // Pick the phase material
            let phase_material = match new_phase {
                1 => boss_assets.material_phase1.clone(),
                2 => boss_assets.material_phase2.clone(),
                3 => boss_assets.material_phase3.clone(),
                _ => boss_assets.material_phase0.clone(),
            };
...
pub fn apply_knockback_effects(
    time: Res<Time>,
    mut commands: Commands,
    mut units: Query<(Entity, &mut Transform, &mut KnockbackEffect)>,
) {
    let delta = time.delta_secs();
