...
/// Fully tears down an in-progress host/connect attempt and resets the lobby to
/// the Connect screen: signals the transport to disconnect, leaves any Steam
/// lobby / P2P socket, resets the `NetworkConnection` (which clears `local_code`,
/// so the connection string is only regenerated by a fresh "Host Game"), and
/// returns the lobby UI to `Connect`.
///
/// Shared by the explicit Cancel/Retry/Disconnect button action and by
/// `cancel_host_on_tab_leave` (host navigating away from the multiplayer tabs
/// while still waiting for a guest).
pub(crate) fn cancel_host_connection(
    ...
    if let (Some(client), Some(lobby_state)) = (steam_client, steam_lobby) {
        leave_steam_lobby(client, lobby_state);
    }
    if let Some(socket) = steam_socket {
