//! Pause menu settings screen systems.
...
use super::components::OnPauseSettingsScreen;
use crate::ui::main_menu::settings::components::{
    ButtonColors, OptionButtonValue, ScrollableContainer, SelectedOption, SettingsButtonAction,
    SliderDownButton, SliderFill, SliderHandle, SliderText, SliderTrack, SliderUpButton,
    SliderValue,
};
use crate::ui::main_menu::settings::styles::{
...
/// * `settings_items` - Query for all entities with the `OnPauseSettingsScreen` marker
pub fn cleanup(mut commands: Commands, settings_items: Query<Entity, With<OnPauseSettingsScreen>>) {
    for entity in &settings_items {
        commands.entity(entity).despawn();
    }
}

/// Handles keyboard input in the pause menu settings.
///
...
pub fn option_button_action(
    interactions: Query<(&Interaction, &OptionButtonValue), Changed<Interaction>>,
...
pub fn slider_button_action(
    down_buttons: Query<(&Interaction, &SliderDownButton), Changed<Interaction>>,
    up_buttons: Query<(&Interaction, &SliderUpButton), Changed<Interaction>>,
    mut game_config: ResMut<GameConfig>,
) {
