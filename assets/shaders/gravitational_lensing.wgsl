...
fn fragment(in: FullscreenVertexOutput) -> @location(0) vec4<f32> {
    // Early out: no active black holes — passthrough.
    // (Branchless UV distortion still works, but skip center darkening logic.)

    // Gravitational lensing: pull UVs toward black hole centers (branchless).
    let lens_is_active_0 = step(0.5, settings.lensing_count);
    let lens_is_active_1 = step(1.5, settings.lensing_count);
    let lens_0 = lensing_offset(in.uv, vec2<f32>(settings.lensing_0_x, settings.lensing_0_y), settings.lensing_0_radius, lens_is_active_0);
    let lens_1 = lensing_offset(in.uv, vec2<f32>(settings.lensing_1_x, settings.lensing_1_y), settings.lensing_1_radius, lens_is_active_1);
    let lensed_uv = clamp(in.uv + lens_0 + lens_1, vec2<f32>(0.0, 0.0), vec2<f32>(1.0, 1.0));
    ...
    // Darken center of black holes to pure black.
