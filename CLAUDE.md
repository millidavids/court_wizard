...
/game-release main   → push main → release.yml      → tag + GitHub Release from that build
                                 → steam-promote.yml → request Steam `default`, then announce
```
...
- **Announcing is a separate, scheduled workflow.** `steam-promote.yml` (`23 */6 * * *`) requests the promotion, waits for the phone tap, and only then posts to Discord and Bluesky. State lives in two git tags — `promoted/` and `announced/` — so nothing repeats.
- **The Steam event is posted by hand, and that is deliberate.** Steam has no supported events API: `ISteamNews` is read-only and `app_build.vdf` has no patch-notes key. Driving the editor's undocumented endpoint was tried and abandoned — `steam/README.md` records the findings so nobody repeats the attempt blind. Instead `steam-promote.yml` renders the event's fields to its run summary, one block per form field, ready to paste. `scripts/changelog_to_bbcode.sh` with no arguments still produces the whole-section BBCode that `release.yml` uploads as an artifact; that no-argument output is load-bearing — don't change it.
