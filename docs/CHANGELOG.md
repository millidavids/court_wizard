...
- **The Lightning Rod's Storm Spire upgrade now holds the towers up as long as you concentrate** — instead of the rods fading on a timer, with Storm Spire they stay (reserving a little of your mana) until you choose to end concentration. Works in single-player and multiplayer.
...
- **Fireball and Meteor explosions now appear on your opponent's screen** — when one player's Fireball or Meteor hit, the other player saw the projectile fly in but no explosion (Meteor just showed an old orange circle). Both explosions now play in full with their sparks and smoke, and the burning ground a meteor leaves behind shows its flames instead of a flat orange patch — all for both players.
...
- **Cast flares and one-shot spell visuals now play on both players' screens for every spell** — the colored flare that bursts from your wizard on each cast, the expanding aura bubbles for Sleep, Haste, Battle Hymn, Berserker Rage, Guardian Circle, Raise the Dead, and Teleport, the smoke poofs for Banishment and Polymorph, the floating motes for Entangle and Healing Plume, the Banishment lensing sphere, the Wall of Stone collapse dust, the Berserker Final Stand explosion sphere, and the doom-skull Mark of Death indicator were all local-only before. Everyone now sees them
- **Brute and Ogre boulders now appear on the other player's screen** — flying boulders thrown by brutes and the ogre boss, plus any pre-placed map boulders, were completely invisible on the guest's side. Both players now see every boulder fly, land, and persist as an obstacle
- **Unit shadows, swaying trees, and pond reactions now appear for both players** — the other player's units used to have no ground shadows, trees and bushes stood perfectly still, and ponds didn't ripple, fog up, or show frost tint when frozen. All three now match single-player on both sides
- **Heat shimmer, black-hole lensing, and teleport ripple distortions now appear in multiplayer** — these post-processing screen-warping effects only ran in single-player, leaving multiplayer fights looking flat. They now distort the view for both players
- **Fire and aura shader animations now play for both players** — fire glow shimmer, aura sphere ripples, and similar live shader effects were frozen on the guest's screen. They now animate at the same rate for both peers
- **Aim line and spell range circle now work in multiplayer** — the wizard's range indicator and the line showing where your spell will land only rendered in single-player. Both now display correctly for each player in multiplayer matches
- **Multiplayer Warglock, Meteorologist, Arcanorouter, and Swordcerer now initialize correctly** — these wizard classes had setup hooks tied to single-player only, so Warglock's guns failed to load, Meteorologist's weather overlay never appeared, Arcanorouter didn't reset between rounds, and Swordcerer's Enter the Fray button never spawned. All four now work in multiplayer too
- **Multiplayer connection refuses mismatched game versions with a clear error** — joining a match against a player on a different game version previously risked silent state corruption or a crash. The lobby now exchanges a version handshake first and shows a "Multiplayer version mismatch" message if you and your opponent aren't on the same version
- **Spell hit particle counts now match between players** — smoke poofs, sparks, mote bursts, and crumble dust were using a fixed count on the receiving side that didn't match what the caster spawned, so the same cast looked subtly different to each player. Counts now travel with the cast event and both players see the same burst
...
- **Multiplayer attackers charge from the moment the match starts** — the opposing army was being treated as if it were in single-player's slow staging phase, so they marched at the off-screen speed used while waves form. Multiplayer armies are now active from spawn and head straight for their targets
- **Disconnects are now caught everywhere** — if your opponent drops their connection at any point, the lobby, loading screen, and live match all detect it cleanly and send you back to the right place with a clear message. Graceful disconnects (when the other player just closes the game) used to leave you stuck in the match with a frozen battlefield; now they trigger the same disconnect overlay as any other connection loss
- **No more leftover battlefield in the wizard tower after a dropped match** — if the connection died while the multiplayer arena was still loading, the half-built terrain and units would linger in the wizard tower scene. The arena now fully cleans up before you're returned to the tower
- **Multiplayer king's guards now attack at the same speed as in single-player** — the elite attack-speed bonus that applies to king's guards in single-player wasn't being given to either side's guards in multiplayer, leaving them swinging slower than expected
- **Walking animation no longer flashes between idle and stride** — the other player's units would slide across the field with their walk cycle flicking back to the idle frame every other tick. Their animation now plays smoothly while they move
- **Clicking Join no longer immediately reports a disconnect** — the lobby was treating the brief moment between clicking Join and the network coming up as a connection loss and dumping the player straight to a failure screen. The lobby now waits for the link to actually come up (or fail) before showing any error
...
- **Spells from one player now actually damage the other player's units** — fireballs and similar attacks cast by the guest were lighting up visually on both screens but never dealt damage to the host's army. Spells now correctly land on the other side, including any leftover effects like burning over time
- **Fire damage shows the burning effect on both players' screens** — the smoke-and-spark visual that plays while a unit is on fire now appears on both players, not just the side that cast the fire spell
- **Multiplayer fireballs match the look of single-player** — the other player's fireball used to render as a slightly off, scale-mismatched ghost without its orange glow halo. It now uses the exact same projectile + glow visual you see when you cast it yourself
- **Excremage's poop conversion now works in multiplayer for the guest player** — picking the Excremage wizard correctly turns the guest's spells into poop damage on enemy units, matching how it behaves in single-player
- **Friendly-fire fire damage on your own wizard now actually burns** — catching your own wizard in your fireball used to deal the initial hit but never apply the lingering burn over time. The burn now stacks correctly
- **Melee units no longer instantly delete each other** — when two armies clashed, the attack-cooldown check let units swing every single frame after their first strike instead of once per cycle. Whole units were being deleted in a fraction of a second. Each unit's swing now lands exactly when its turn in the attack rotation comes around, the same pace as single-player. Also fixes a frame-rate dependency where the bug was much worse on higher-refresh displays
- **Other player's unit animations now stay smooth** — the walking and attack animations on the opposing army would flicker back to the idle pose every time a unit briefly paused. The host now sends each unit's actual motion to the other player, so the animation looks identical on both screens
- **King's guards now appear and take damage on the other player's screen** — the four elite guards next to each king were silently missing from the network snapshot, so the other player never saw them spawn, move, or die. They now sync just like the rest of the army
...
- **Other player's units now animate without restarting every frame** — the walking and attack animations on the other player's side were being reset back to their starting frame on every single network update, leaving units looking glitchy and frozen in place. Animations now play through to completion just like they do on your own side
- **Death animations on the other player's units no longer mess up nearby corpses** — when a unit died on the other player's side, its death sprite frames were being written into a shared texture slot, which could momentarily swap the appearance of other already-fallen corpses to a dying pose. Each dying unit now animates in its own material so nearby corpses stay still
- **Units already in mid-action now appear correctly the moment you see them** — when a fresh army first showed up on the other player's screen (or when you joined an in-progress match), units that were already burning, frozen, electrified, or mid-swing would wait one extra update before their visuals appeared. They now show up in their current state from the very first frame
- **King's aura matches single-player in multiplayer** — the multiplayer king used to show a flat ground circle (or a translucent shield bubble) to mark his presence. Both players now see the same glowing aura halo around the king that single-player has always shown, on both sides of the arena
- **Trees and bushes catch fire in multiplayer** — fireballs, meteors, walls of fire, ground fires, and disintegrate beams now ignite trees and bushes on both players' screens. The foliage burns, smokes, and sparks for everyone — and damage to nearby units stays consistent on both sides
- **King's protective shield now drops correctly in the late game** — in multiplayer, the king's spell-immunity shield never actually dropped when his defending army was nearly wiped out, leaving him permanently spell-proof for the whole match. Both kings now lose their shield once their own team's army falls below 10%, opening them up to direct spell damage the way single-player intends
- **Dead king no longer glows** — when a king fell, his glowing aura halo used to keep shining around his fallen sprite. The aura now vanishes the moment the king dies, on every screen
