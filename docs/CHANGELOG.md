...
- **Exiting the game no longer freezes for several seconds (or freezes the terminal)** — clicking Exit could hang the application and the terminal that launched it while the multiplayer network connection tried to close cleanly. Exit now completes quickly regardless of network state
